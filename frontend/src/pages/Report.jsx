    ...
    <Layout>
      <div className="animate-in fade-in duration-500 max-w-5xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-blue-900 text-white p-10 rounded-[2rem] shadow-2xl mb-10 relative overflow-hidden">
          <div className="absolute right-0 top-0 w-64 h-64 bg-white/5 rounded-full blur-3xl pointer-events-none transform translate-x-1/2 -translate-y-1/2"></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
          ...
          {/* Consistency Stats */}
          <div className="lg:col-span-5 bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-6">
              <div className="bg-blue-50/50 border border-blue-100 p-6 rounded-2xl relative overflow-hidden">
                <div className="flex justify-between items-end mb-4 relative z-10">
                  ...
                  <span className="text-4xl font-extrabold text-blue-600">{consistency.consistency}%</span>
                </div>
              ...
              <div className="bg-purple-50/50 border border-purple-100 p-6 rounded-2xl">
                <div className="flex justify-between items-end mb-4">
                  ...
                  <div className="text-right">
                    <span className="text-2xl font-extrabold text-purple-600">{consistency.completed}</span>
                    <span className="text-purple-400 font-medium"> / {consistency.weeklyGoal} days</span>
                  </div>
                </div>
                <div className="w-full bg-purple-100 rounded-full h-4">
                  <div 
                    ...
                    style={{ width: `${((consistency.completed || 4) / (consistency.weeklyGoal || 5)) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-2xl flex items-start gap-4">
                ...
                <div>
                  <p className="text-emerald-800 font-bold mb-1">High Momentum</p>
          ...
          {/* Skills Breakdown */}
          <div className="lg:col-span-7 bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              <FiTarget className="text-indigo-500" /> Competency Breakdown
            </h3>
            
            <div className="space-y-5">
              <div className="group border border-emerald-100 hover:border-emerald-300 p-5 rounded-2xl transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {report.strong.map((skill, index) => (
                    <span key={index} className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-1.5 rounded-full text-sm font-bold shadow-sm group-hover:-translate-y-0.5 transition-transform">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div className="group border border-amber-100 hover:border-amber-300 p-5 rounded-2xl transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {report.medium.map((skill, index) => (
                    <span key={index} className="bg-amber-50 border border-amber-200 text-amber-700 px-4 py-1.5 rounded-full text-sm font-bold shadow-sm group-hover:-translate-y-0.5 transition-transform">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div className="group border border-red-100 hover:border-red-300 p-5 rounded-2xl transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {report.weak.map((skill, index) => (
                    <span key={index} className="bg-red-50 border border-red-200 text-red-700 px-4 py-1.5 rounded-full text-sm font-bold shadow-sm group-hover:-translate-y-0.5 transition-transform">
                      {skill}
                    </span>
                  ))}
                </div>
